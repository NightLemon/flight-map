...
前端默认通过开发服务器的同源 `/api` 代理访问 API。也可在启动或构建前设置 `$env:VITE_API_BASE_URL = "http://127.0.0.1:8000"`，JSON 和 PDF 请求会使用同一基地址；填 API 服务根地址，不追加 `/api/v1`。基地址可保留反向代理路径前缀，尾斜杠会统一处理。该配置不修改后端 CORS；直接跨源访问时前端仍需使用已允许的 `http://127.0.0.1:5173` 或 `http://localhost:5173`。
...
Windows 清理副本时，仅对短暂共享/锁定占用（WinError 32/33）最多尝试 4 次，总等待 0.7 秒；每次删除前重新确认目标目录边界。持续占用或其他错误仍使整体报告失败，不能将“数据断言通过”误记为完整演练通过。详细记录见 [接手复核](takeover-2026-09-08.md)。
